...
- **After merging PRs #425 and #443 (merge commit `ecd8b57`, 2026-09-23):** voice UI is #443's Stage / FloatCard / DockZonesOverlay / StageSplit, with layout state in `VoiceContext`. `VoiceBottomBar.tsx` is now a thin lazy wrapper, and the bar's UI lives in `components/Voice/VoiceBottomBarContent.tsx`. Voice components are lazy-loaded and livekit is imported dynamically. **Never add a static runtime import of `livekit-client` or of the `components/Voice` barrel to anything on the initial render path.** Use `features/voice/livekitEvents.ts` constants and direct file imports. The **baseline (before) screenshots for voice and channel stories** are now `frontend/.ux-shots/post-merge/` and `post-merge-channel/`, not `final/`, because #443 intentionally changed them. Every gate must run `docker compose run --rm frontend sh -c 'pnpm run build && pnpm run check:bundle-budget'`, and it must pass.
...
### Task 16: Voice on the phone (building on #443)
**Context:** #443 already provides a tile per participant (avatar, name and speaking ring for people with no camera), a community Stage, a desktop FloatCard and a desktop DM StageSplit. Do NOT reintroduce a separate participant list or rebuild the stage. Improve what #443 left open:
**Files:** `components/Voice/VoiceBottomBarContent.tsx` (phone: primary controls are mic, deafen, camera and hang-up, with screen share, soundboard, settings and the rest in a "more" `MobileSheet`. It must fit 320px with no clipping; right now the speaker icon sits under the mic and hang-up is cut off), `components/Voice/VideoTiles.tsx` (phone density: switch to compact avatar tiles or a 3-column layout above about 6 participants so 25 people stay legible; tile names on one line with an ellipsis everywhere), `components/Voice/FloatCard.tsx` (ellipsis on the label, no overlapping long names), `hooks/useStagePresence.ts` plus the tablet content area (the tablet chat panel must report the stage as mounted so the float card hides instead of repeating a tile). Check the phone "show tiles" full-screen sheet for safe area and 44px close button.
- [ ] Tests: the bar renders 4 primary controls plus "more" on phone, and "more" holds the rest; desktop bar unchanged (Electron mocked too); VideoTiles switches density above the threshold; the name has `noWrap` and an ellipsis; on tablet the float card is hidden when the stage is mounted.
- [ ] Acceptance: the `edge-voice-channel--four-mixed-states`, `twenty-five-connected`, `worst-case` and `video-overlay-cameras` shots (phone and tablet), and the bundle budget still passes.
...
### Task 20: "Maximum update depth exceeded" loop (runs alone, after Task 19)
**Context:** this happens intermittently in `edge-chat-worst-case--everything-with-thread-open` and `edge-nav-worst-case--channel-members` (55+ members, 60+ friends). A gate traced it to a TanStack Query store notification loop that existed before this work. An earlier code check found that `useAuthenticatedFile.ts:51,56` calls setState unconditionally inside an effect, and that `AvatarCacheContext.tsx:183` has a context value that isn't memoized.
**Files:** find them from the repro. Start with `hooks/useAuthenticatedFile.ts`, `contexts/AvatarCacheContext.tsx`, `components/Common/UserAvatar.tsx` and any `useUser`/`useSyncExternalStore` per-avatar subscription.
- [ ] Reproduce it reliably: a Vitest test rendering 60+ avatars or members, or a story run 5 times. Find the root cause (use the systematic-debugging approach) and fix it at the source.
- [ ] Tests: a regression test that renders 250 members without the React error.
- [ ] Acceptance: both stories run 5 times in a row with no page error.
