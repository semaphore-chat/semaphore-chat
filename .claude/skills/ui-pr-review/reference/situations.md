...
- **An app timer that fires around the time of the shot.** The jump highlight clears 3 s after a `?highlight=` jump and re-renders the target row, and `edge-chat-history--jump-far-back` was caught doing that once. Its driver now waits the flash out (`wait(3500)`), and `cold-deep-link` does the same.
- **Story set-up that the app undoes, or that misses its target.** A refetch dropped the optimistic rows `useInjectOptimisticMessages` had put in (`edge-chat-dm--dm-send-states`), and a click on "200 replies" found nothing once the list had kept to the bottom and that row was outside the rendered window (`edge-chat-worst-case--everything-with-thread-open` on phone). The hook now marks the page busy until the rows are in and puts them back after a refetch. The driver scrolls up until the row renders.
