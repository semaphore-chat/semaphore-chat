...
- A run takes **2 to 11 minutes**. A single screen with its own story takes about 2 minutes. A broad change takes about 6 minutes of render probe and about 10 minutes in total. The first run on a new dependency set also builds a `uir-frontend:<hash>` image. That is longer than a foreground command may take, so start it in the background (Bash `run_in_background`) and continue when it finishes.
- **Outputs:**
...
Cover the edge cases your change can break: long and unbroken names, empty lists, many items, loading, errors, unread and mention badges, dark and light. How to write them, the helpers, and the determinism rules are in **[reference/stories.md](reference/stories.md)**.

Type-check and lint the new files, then go on to step 3. New stories show up as **new**, with after-only composites.

## 3. Capture before/after

Rerun step 1's command whenever you added stories or changed code. For quick iterations on a few stories, `--stories id1,id2` captures exactly those, in about 1.5 minutes for four stories. But finish with the normal run on committed code, because that is the run the PR section should come from.

## 4. Look at every screenshot

Open the images with the **Read tool** (it displays `.webp` and `.png`). Look at:

- **every changed, new and removed composite at phone, tablet and desktop.** Stories whose id contains `keyboard` have only `phone-short` (390×500).
- **every unstable composite.**
- **a sample of unchanged stories**: two or three that run your changed files (their `reasons` in `report.json`) plus one elsewhere. Open their head shots at each viewport in `.ui-review/shots/head/<viewport>/<id>.png`. An "unchanged" result for code you meant to change visibly is a finding in itself.
...
1. **Intended?** Every changed story is one you meant to change. An unexpected change is a regression until you can explain it.
...
**New PR:** do steps 1 to 5 on committed code first. Then:
...
- **Housekeeping:**
  ...
  - Never commit images.
  - For stacked PRs, use `--base origin/<parent-branch>`.
  - `uir-frontend:<hash>` images take about 3.7 GB each and are never deleted by the tool.
