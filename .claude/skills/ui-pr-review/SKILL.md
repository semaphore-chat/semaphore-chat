...
- **Flaky stories:** each changed shot is captured twice more, and a difference that doesn't reproduce is reported as **unstable**, not changed. No story is known to be flaky. A story you write has to follow the determinism rules in reference/stories.md: `Date.now()` doesn't move during a review, so drive interactions with `useDriver` and its `wait()`/`mediaSettled()` steps.
- **Housekeeping:**
