...
8. **Freeze time with `context.clock.install({ time: SHOWCASE_NOW })` and `timezoneId: 'America/New_York'`.** Then "9:04 PM" in `showcase.ts` renders as 9:04 PM and "Today" stays today. Timers still run in real time, so animations and scripted delays work. `SHOWCASE_NOW` is defined in both `lib.mjs` and `showcase.ts`, and the two must match.
...
26. **The animated WebP encoder skips pixels that changed by only a few levels.** A slow, linear crossfade over dark content never finishes in those areas, so the old scene's text stays burnt into the new scene as dark blocks. The hero's crossfades use a steep ease-in (`HERO_XFADE`, a sixth-power curve) so the last step is large enough to be encoded. A cubic curve was enough over busy content, but left a dark ghost of the thread panel in the empty corner of the three-tile Squad Up stage, where the two scenes differ by only a few levels.
...
## The message list and GIFs

33. **The message list is virtualised.** Rows far above the fold aren't in the DOM, so `scrollIntoViewIfNeeded()` on "5 replies" waits until it times out. `chatScene` rests the cursor on the composer and scrolls the list up (`scrollUntilVisible` in `record.mjs`) before it goes to click.
34. **A reaction added to the newest message stays below the fold.** The list doesn't follow the last row as it grows, so the new chip is hidden under the composer. Scenes don't react to the message just sent. This is a product quirk; report it rather than work around it with data.
35. **No row may be cut at the top of `chat-desktop` or `chat-phone`**, and where the top edge falls depends on the heights of everything below it. The pit screenshot's size (523×257, a crop wider than 16:9, in `pitScreenshotSvg`) is tuned so both open with a clean top edge: on desktop the image shows at its natural height, on a phone it is scaled to the column width, so its height moves the desktop edge and its width the phone one. If you change #general, check both top edges again.
36. **GIFs are real GIFs, made in the page** (`showcaseGifs.ts`): each is drawn on a canvas, quantised to 256 colours and encoded as a GIF89a on first request. MSW answers `/api/gifs/search` and `/api/gifs/featured` (the backend's provider proxy) and serves the files from `https://media.gifs.example`; its service worker intercepts cross-origin image requests too. A GIF message is just the URL, which must end in `.gif` for the app to embed it (`getLoneGifUrl`). The showcase turns `gifSearchEnabled` on in `/api/instance/settings/public`, so the composer shows its GIF button in every shot.
37. **Presence comes from `SHOWCASE_ONLINE_IDS`, not from `status`.** The base handlers treat "has a status" as online, but most people online in the showcase haven't set one. `presenceHandlers` in `showcaseStory.ts` overrides them.
